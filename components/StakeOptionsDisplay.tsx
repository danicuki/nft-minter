...
export const StakeOptionsDisplay = ({
  ...
  const sendAndConfirmTransaction = useCallback(
    ...
    [checkStakingStatus, walletAdapter, connection]
  )
  ...
  }, [walletAdapter.connected, walletAdapter.publicKey, nftTokenAccount, connection, nftData, sendAndConfirmTransaction])
    ...
    >
      ...
      <VStack spacing={-1}>
        <Text color="white" as="b" fontSize="4xl">
          {isStaking ? `${totalEarned} $W3D` : "0 $W3D"}
        </Text>
        <Text color="bodyText">
          {isStaking ? `${claimable} $W3D earned` : "earn $W3D by staking"}
        </Text>
      </VStack>
      <Button
        ...
        width="200px"
      >
        <Text as="b">{isStaking ? "claim $W3D" : "stake WEB3DEVER"}</Text>
      </Button>
